@extends('layouts.app')
@section('title', 'Buat Siklus - BukuTernak')
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <form method="POST" action="{{ route('cycles.store') }}" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                <input name="name" type="text" placeholder="Contoh: Ayam Batch 1" required value="{{ old('name') }}"
            ...
            <div class="space-y-1.5">
                ...
                <select name="livestock_type" required
                    ...
                    @foreach($livestockTypes as $type)
                        <option value="{{ $type }}" {{ old('livestock_type') === $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Tanggal Mulai</label>
                <input name="start_date" type="date" required value="{{ old('start_date', date('Y-m-d')) }}"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Jumlah Awal (ekor)</label>
                <input name="initial_count" type="number" min="1" placeholder="100" required value="{{ old('initial_count') }}"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Modal Awal (Rp)</label>
                <input name="initial_capital" type="number" min="0" step="0.01" placeholder="5000000" required value="{{ old('initial_capital', 0) }}"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
