@extends('layouts.app')
@section('title', 'Tambah Biaya - BukuTernak')
...
<div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <form method="POST" action="{{ route('cycles.expenses.store', $cycle) }}" class="space-y-4">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <select name="category" required class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a] bg-white">
                    ...
                    <option value="obat_vitamin" {{ old('category') === 'obat_vitamin' ? 'selected' : '' }}>Obat / Vitamin</option>
            ...
            <div class="space-y-1.5">
                ...
                <input name="amount" type="number" min="0" placeholder="50000" required value="{{ old('amount') }}"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
            <div class="space-y-1.5">
