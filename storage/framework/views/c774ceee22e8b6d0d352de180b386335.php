<?php $__env->startSection('title', 'Tambah Pakan - BukuTernak'); ?>
...
<div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <form method="POST" action="<?php echo e(route('cycles.feed.store', $cycle)); ?>" class="space-y-4">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="space-y-1.5">
                    ...
                    <input name="quantity" type="number" step="0.01" min="0.01" placeholder="10" required value="<?php echo e(old('quantity')); ?>"
                        class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
                </div>
                <div class="space-y-1.5">
                    ...
                    <select name="unit" class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a] bg-white">
                        <option value="kg" <?php echo e(old('unit', 'kg') === 'kg' ? 'selected' : ''); ?>>Kg</option>
                        <option value="karung" <?php echo e(old('unit') === 'karung' ? 'selected' : ''); ?>>Karung</option>
                        <option value="sak" <?php echo e(old('unit') === 'sak' ? 'selected' : ''); ?>>Sak</option>
                    </select>
                </div>
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Biaya (Rp)</label>
                <input name="cost" type="number" min="0" placeholder="150000" required value="<?php echo e(old('cost')); ?>"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
            <div class="space-y-1.5">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/fataorgana/Project/buku-ternak/resources/views/cycles/feed/create.blade.php ENDPATH**/ ?>