<?php $__env->startSection('title', 'Edit Siklus - BukuTernak'); ?>
...
<div>
    <div class="flex items-center gap-2 mb-5">
        ...
        <div>
            ...
            <p class="text-xs text-gray-500"><?php echo e($cycle->name); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <form method="POST" action="<?php echo e(route('cycles.update', $cycle)); ?>" class="space-y-4">
            <?php echo csrf_field(); ?>
            <?php echo method_field('PUT'); ?>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Nama Siklus</label>
                <input name="name" type="text" placeholder="Contoh: Ayam Batch 1" required value="<?php echo e(old('name', $cycle->name)); ?>"
            ...
            <div class="space-y-1.5">
                ...
                <select name="livestock_type" required
                    ...
                    <?php $__currentLoopData = $livestockTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($type); ?>" <?php echo e(old('livestock_type', $cycle->livestock_type) === $type ? 'selected' : ''); ?>><?php echo e($type); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Tanggal Mulai</label>
                <input name="start_date" type="date" required value="<?php echo e(old('start_date', $cycle->start_date->format('Y-m-d'))); ?>"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Jumlah Awal (ekor)</label>
                <input name="initial_count" type="number" min="1" placeholder="100" required value="<?php echo e(old('initial_count', $cycle->initial_count)); ?>"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
            <div class="space-y-1.5">
                <label class="block text-sm font-medium text-gray-700">Modal Awal (Rp)</label>
                <input name="initial_capital" type="number" min="0" step="0.01" placeholder="5000000" required value="<?php echo e(old('initial_capital', $cycle->initial_capital)); ?>"
                    class="w-full h-12 rounded-xl border border-gray-200 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#3c8d5a]" />
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/fataorgana/Project/buku-ternak/resources/views/cycles/edit.blade.php ENDPATH**/ ?>